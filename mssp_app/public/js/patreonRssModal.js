export function createPatreonRssModal({ dom, patreonSources, getEpisodes, onSourcesChanged }) {
  ...
  function open(trigger = document.activeElement) {
    ...
    setRevealed(false);
    setStatus("");
    dom.patreonRssTitle.textContent = storedUrl ? "Manage Patreon RSS" : "Connect Patreon RSS";
    dom.patreonRssSubmit.textContent = storedUrl ? "Replace" : "Connect";
    dom.patreonRssRemove.hidden = !storedUrl;
  ...
  function setBusy(value) {
    ...
    dom.patreonRssSubmit.textContent = busy ? "Connecting…" : (patreonSources.getStoredUrl() ? "Replace" : "Connect");
  }
